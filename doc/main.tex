...
Еще одним примером интегрируемой системы является Волчок Эйлера --- это модель, в которой точка опоры является и центром масс волчка. Волчок вращается вокруг этой неподвижной точки и на него не действует никаких внешних сил.

Также существуют работы, учитывающие трение в ножке волчка. В работе [2] предсказывается стабилизация или падение изначально наклоненного, закрученного волчка, с помощью системы инерциальных моментов действующих на волчок. 

В отличие от указанных подходов, в данной работе рассматривается иная задача. 
Моделируется волчок, ножка которого опирается не на закреплённый шарнир, а на идеально гладкую горизонтальную поверхность, что позволяет ножке скользить по ней при вращении в силу отсутствия сил трения в ножке, а центру масс --- оставаться на месте. 
...
\begin{equation}
N_z = \dfrac{g - \left[ \left( \bigl( \boldsymbol{\omega}^* \mathbf{I}^{-1} + \mathbf{I}^{-1} \boldsymbol{\omega}^{*T} \bigr) \mathbf{L} \right) \times \boldsymbol{\rho} + \boldsymbol{\omega} \times (\boldsymbol{\omega} \times \boldsymbol{\rho}) \right]_z}{\dfrac{1}{m} + \left[ \left( \mathbf{I}^{-1} (\boldsymbol{\rho} \times \hat{\mathbf{z}}) \right) \times \boldsymbol{\rho} \right]_z}
\end{equation}
